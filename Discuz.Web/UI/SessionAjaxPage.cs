...
namespace Discuz.Web.UI
{
    ...
    public class SessionAjaxPage : PageBase
    {
        ...
        private void GetImageList()
        {
            ...
            foreach (AttachmentInfo info in attachList)
            {
                sb.AppendFormat("{{'aid':{0},'attachment':'{1}','attachkey':'{2}','description':''}},", info.Aid, info.Attachment.Replace("'", "\\'"), GetKey(info.Aid));
            }
            ResponseJSON(sb.ToString().TrimEnd(',') + "]");
        ...
        private void QuickReply()
        {
            ...
            int postDisabled = 0;
            if (admininfo != null)
                postDisabled = admininfo.Disablepostctrl;
            else
                postDisabled = config.Disablepostad;
            ...
            //获取postid
            postid = DNTRequest.GetInt("postpid", -1);
            int replyuserid = 0;
