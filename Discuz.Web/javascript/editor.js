...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		menu.innerHTML = '<span class="y"><a onclick="hideMenu()" class="flbc" href="javascript:;">关闭</a></span><div class="p_opt cl"><div>' + str + '</div><div class="pns mtn"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>提交</strong></button><button type="submit" onClick="hideMenu()" class="pn"><em>取消</em></button></div></div>';
...
function getunusedattachlist_callback(doc) {
    ...
    if (data.length > 0) {
        ...
        var msg = '';
        msg += '<form id="unusedform"><div style="overflow-y: auto; overflow-x: hidden;" class="c ufl">';
