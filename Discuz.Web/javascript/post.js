...
function getreadpermoption(obj, selectvalue) {
    if (!$('topicreadperm'))
        return;
    var readpermoption = $('topicreadperm').options;
    obj.options.length = 0;
    for (i = 0; i < readpermoption.length; i++) {
        var optionItem = document.createElement('OPTION');
        optionItem.title = readpermoption[i].title;
        optionItem.value = readpermoption[i].value;
        optionItem.text = readpermoption[i].innerHTML;
