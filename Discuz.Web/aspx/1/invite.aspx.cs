    ...
    public class invite : PageBase
    {
        ...
        public void SendEmail()
        {
            ...
            foreach (string address in emailAddress)
            {
                ...
                if (Emails.SendEmailNotify(address, "来自您的好友:" + invitecodeinfo.Creator + "的邀请!", string.Format(invitationconfiginfo.InvitationEmailTemplate, address, userid, invitecodeinfo.Creator, GetUserInviteUrl(invitecodeinfo.Code, true), config.Forumtitle, usersaid != "" ? usersaid : "")))
                    sendSuccessCount++;
        ...
        public void CreateEmailPreview()
        {
            ...
            emailpreview = string.Format(invitationconfiginfo.InvitationEmailTemplate, "[friend]", userid, invitecodeinfo.Creator, GetUserInviteUrl(invitecodeinfo.Code, true), config.Forumtitle, usersaid);
        }

        public string InviteCodeExpireTip(string time)
        {
            int dateDiff = Utils.StrDateDiffHours(time, 0);
            if (dateDiff < 0)
            {
                string script = "";
                switch (dateDiff / -24)
