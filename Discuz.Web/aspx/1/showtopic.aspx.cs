        ...
        public new void BindPageCountAndId()
        {
            ...
            if (pageid != pagecount)
                nextpage = "<a href=\"" + (string.IsNullOrEmpty(onlyauthor) || onlyauthor == "0" ? Urls.ShowTopicAspxRewrite(topicid, pageid + 1) :
                    forumpath + string.Format("showtopic.aspx?onlyauthor={0}&topicid={1}&forumpage={2}&posterid={3}&page={4}", onlyauthor, topicid, forumpageid, posterid, pageid + 1)) + "\" class=\"next\">下一页</a>";
