...
override protected void OnInit(EventArgs e)
{
	...
	*/
	...
	templateBuilder.Append("\r\n				<li><a href=\"announcement.aspx#" + announcement["id"].ToString().Trim() + "\" class=\"xg2\">" + announcement["title"].ToString().Trim() + "<em>");	templateBuilder.Append(Convert.ToDateTime(announcement["starttime"].ToString().Trim()).ToString(" yyyy-MM-dd HH:mm"));
	...
	int hotpics__loop__id=0;
	foreach(DataRow hotpics in forumhotslist.Rows)
	{
		hotpics__loop__id++;

	string datatype = hotpics["datatype"].ToString();
	
	string sorttype = hotpics["sorttype"].ToString();
	
	string forumid = hotpics["forumid"].ToString();
	
	int topiccount = TypeConverter.ObjectToInt(hotpics["topiccount"]);
	
	int cachetime = TypeConverter.ObjectToInt(hotpics["cachetime"]);
	
	int id = TypeConverter.ObjectToInt(hotpics["id"]);
	
	int displayorder = TypeConverter.ObjectToInt(hotpics["displayorder"]);
	
	int topictitlelength = TypeConverter.ObjectToInt(hotpics["topictitlelength"]);
	

	if (datatype=="pictures")
	{

	templateBuilder.Append("\r\n			            <div class=\"title_bar xg2\">\r\n				            <h2>" + hotpics["name"].ToString().Trim() + "</h2>\r\n			            </div>\r\n                        <div id=\"focusViwer\">\r\n                            <div id=\"imgADPlayer\"></div> \r\n                            <script  type=\"text/javascript\">\r\n                                var hotimagesarray = eval('");
	templateBuilder.Append(ForumHots.HotImagesArray(topiccount,cachetime,sorttype,id,forumid,displayorder).ToString().Trim());
	templateBuilder.Append("');\r\n                                for (i = 0; i < hotimagesarray.length; i++) {\r\n                                    PImgPlayer.addItem(\"\" + hotimagesarray[i].title.substr(0,");
	templateBuilder.Append(topictitlelength.ToString());
	templateBuilder.Append(") + \"\", \"\" + hotimagesarray[i].url + \"\", \"\" + hotimagesarray[i].img + \"\");\r\n                                }\r\n                                if(hotimagesarray.length>0)\r\n                                    PImgPlayer.init(\"imgADPlayer\", 360, 240);   \r\n                            </");
	templateBuilder.Append("script>\r\n                        </div>\r\n			        ");
	}	//end if
	...
	int forumhots__loop__id=0;
	foreach(DataRow forumhots in forumhotslist.Rows)
	{
		forumhots__loop__id++;

	int displayorder = TypeConverter.ObjectToInt(forumhots["displayorder"]);
	

	if (forumhots["id"].ToString().Trim()!="6" && displayorder==1)
	{

	templateBuilder.Append("\r\n				                <li class=\"switchNavItem\" index=\"2\" id=\"tab_li_" + forumhots["id"].ToString().Trim() + "\"><a href=\"javascript:;\"  onmousemove=\"tabselect(" + forumhots["id"].ToString().Trim() + ")\">" + forumhots["name"].ToString().Trim() + "</a></li>\r\n				            ");
	}	//end if
	...
	int forumhotname__loop__id=0;
	foreach(DataRow forumhotname in forumhotslist.Rows)
	{
		forumhotname__loop__id++;

	int displayorder = TypeConverter.ObjectToInt(forumhotname["displayorder"]);
	

	if (forumhotname["id"].ToString().Trim()!="6" && displayorder==1)
	{

	string datatype = forumhotname["datatype"].ToString();
	
	int topiccount = TypeConverter.ObjectToInt(forumhotname["topiccount"]);
	
	int forumnamelength = TypeConverter.ObjectToInt(forumhotname["forumnamelength"]);
	
	int topictitlelength = TypeConverter.ObjectToInt(forumhotname["topictitlelength"]);
	
	int cachetime = TypeConverter.ObjectToInt(forumhotname["cachetime"]);
	
	int id = TypeConverter.ObjectToInt(forumhotname["id"]);
	

	if (datatype=="topics")
	{

	templateBuilder.Append("\r\n			            <div class=\"newHotB\" name=\"hot_layer_" + forumhotname["id"].ToString().Trim() + "\" id=\"hot_layer_" + forumhotname["id"].ToString().Trim() + "\"  style=\"display:none\">	\r\n				            ");	TopicOrderType sorttype = (TopicOrderType)Enum.Parse(typeof(TopicOrderType), forumhotname["sorttype"].ToString());
	
	TopicTimeType topictimetype = (TopicTimeType)Enum.Parse(typeof(TopicTimeType), forumhotname["topictimetype"].ToString());
	
	string forumid = forumhotname["forumid"].ToString();
	

	int hottopic__loop__id=0;
	foreach(DataRow hottopic in ForumHots.GetTopicList(topiccount,0,sorttype,false,cachetime,false,forumid,id,topictimetype).Rows)
	{
		hottopic__loop__id++;

	string topicsname = Utils.GetSubString(Utils.RemoveHtml(hottopic["title"].ToString().Trim()),topictitlelength*2,"");
	
	string forumsname = Utils.GetSubString(Utils.RemoveHtml(hottopic["name"].ToString().Trim()),forumnamelength*2,"");
	...
	foreach(DataRow firsttopic in ForumHots.GetFirstPostInfo(tid,cachetime).Rows)
	{
	...
	templateBuilder.Append(forumurl.ToString());
	templateBuilder.Append(aspxrewriteurl.ToString());
	templateBuilder.Append("\" target=\"_blank\" class=\"xg2\">");
	templateBuilder.Append(topicsname.ToString());
	templateBuilder.Append("</a></li>\r\n					            ");
	...
	templateBuilder.Append("\r\n			            <div class=\"newHotB\" name=\"hot_layer_" + forumhotname["id"].ToString().Trim() + "\" id=\"hot_layer_" + forumhotname["id"].ToString().Trim() + "\" style=\"display:none\">	\r\n				            ");	string sorttype = forumhotname["sorttype"].ToString();
	

	int user__loop__id=0;
	foreach(ShortUserInfo user in ForumHots.GetUserList(topiccount,sorttype,cachetime,id))
	{
		user__loop__id++;
	...
	if (sorttype=="thismonth")
	{
	templateBuilder.Append(user.Posts.ToString().Trim());
	}	//end if


	if (sorttype=="digestposts")
	{
	...
	foreach(ForumInfo foruminfo in ForumHots.GetHotForumList(topiccount,sorttype,cachetime,id))
	{
		foruminfo__loop__id++;
	...
	templateBuilder.Append("';\r\nvar lastpostforumhtml='您将要在<a id=\"userselectforum\" href=\"javascript:void(0)\" onmouseover=\"showMenu(this.id)\" class=\"drop\">");
	...
	templateBuilder.Append("</dd>\r\n		</dl>\r\n		<script type=\"text/javascript\">$('loginorreg').innerHTML=lastpostforumhtml;</");
	...
	templateBuilder.Append("\r\n	<li>\r\n		<a href=\"javascript:void(0)\" onClick=\"chooseforums('");
